...
const Card = (props) => {
	...
	useEffect(() => {
		fetch('https://shrouded-wildwood-03121.herokuapp.com/products').then((res) => res.json()).then((data) => {
			setCard(data);
		});
	}, []);
		...
		<div>
			<div className="container card-area">
				<div className="row">
					<nav>
						<ul className="nav justify-content-center">
							<li onClick={() => setFoodCategory('breakfast')} className="nav-item">
								<span
									to="breakfast"
									className={foodCategory === 'breakfast' ? 'active nav-link' : 'nav-link'}
								>
									Breakfast
								</span>
							</li>
							<li onClick={() => setFoodCategory('lunch')} className="nav-item">
								<span
									to="breakfast"
									className={foodCategory === 'lunch' ? 'active nav-link' : 'nav-link'}
								>
									Lunch
								</span>
							</li>
							<li onClick={() => setFoodCategory('dinner')} className="nav-item">
								<span
									to="breakfast"
									className={foodCategory === 'dinner' ? 'active nav-link' : 'nav-link'}
								>
									Dinner
								</span>
							</li>
					...
					<div className="row">
						{cardItems.map((food) => <SingleCard key={food.id} food={food} cardItems={cardItems} />)}
					</div>
